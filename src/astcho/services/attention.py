...
class AttentionService:
    ...
    def __init__(self, bot_id: str, *, bot_name: str = "Astcho", base_talk_probability: float = 0.1):
    ...
    def should_plan(
        ...
        *,
        ...
        direct = message.mentioned_bot or message.replied_to_bot or self.bot_name.lower() in message.text.lower()
        ...
        if self.last_bot_reply and now - self.last_bot_reply < 5:
            return False
        silence_bonus = min(0.2, max(0, now - self.last_bot_reply) / 600 * 0.05) if self.last_bot_reply else 0
        recent = [m for m in self.messages if not m.is_bot and now - m.timestamp <= 300]
        average_gap = ((recent[-1].timestamp - recent[0].timestamp) / (len(recent) - 1)
                       if len(recent) > 1 else 999)
        frequency_factor = 0.6 if average_gap < 60 else 0.75 if average_gap < 180 else 0.9 if average_gap < 300 else 1.0
    ...
    def context(self, limit: int = 20) -> str:
        ...
        for message in list(self.messages)[-limit:]:
            role = self.bot_name if message.user_id == self.bot_id or message.is_bot else message.nickname
