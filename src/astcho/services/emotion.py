...
class EmotionService:
    ...
    def _apply_affinity(self, group_id: str, user_id: str, score: float) -> None:
        ...
        daily = float(user.get("affinity_daily_delta", 0)) if user.get("affinity_day") == day else 0.0
        ...
        self.store.set_affinity_state(group_id, user_id, affinity=affinity,
                                      daily_delta=daily + actual, day=day,
                                      changed_at=now, decayed_at=float(user.get("affinity_decayed_at", now) or now))
    ...
    def _decay_affinity(self, group_id: str, user_id: str, user: dict) -> None:
        ...
        self.store.set_affinity_state(group_id, user_id, affinity=decayed,
                                      daily_delta=float(user.get("affinity_daily_delta", 0)),
                                      day=str(user.get("affinity_day", "")),
                                      changed_at=float(user.get("affinity_changed_at", 0)), decayed_at=now)
...
def apply_typo(text: str, excitement: float, *, random_value: float | None = None) -> str:
    ...
    if sample > (excitement - 0.3) * 0.35:
        return text
    substitutions = {"好耶": "豪耶", "厉害": "利害", "支持": "智齿", "真的": "震的",
                     "开心": "凯心", "谢谢": "鞋鞋", "嘻嘻": "系系"}
